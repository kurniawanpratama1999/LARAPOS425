...
const searching = () => {
    ...
    searchEl.addEventListener('keyup', () => {
        ...
        debounce = setTimeout(async () => {
            ...
            const response = await fetch(`/dashboard/categories/search?q=${encodeURIComponent(val)}`)
...
const yesDeleteRows = async ({param}) => {
    ...
    const response = await fetch(`/dashboard/categories/destroys${param}`, {
        method:"POST",
        headers: {
...
const btnDelete = () => {
    ...
    <div id="asking-to-delete" class="fixed z-200 top-0 bottom-0 max-[1000px]:left-0 left-80 right-0 backdrop-blur-sm bg-black/10 flex items-center justify-center">
        <div class="max-w-[450px] bg-neutral-100 shadow rounded p-5">
            ...
            <p>Apakah kamu yakin akan menghapus data dari category id ${ids.join(" & ")} ?</p>
